...
public class TestPoint {
	...
	public TestPoint(String line) throws Exception {
		...
		if (t.length!=6) 
			throw new Exception("Invalid record from server:" + line + "(" + t.length + ")");
	...
	public Object getCsvData() {
		return this.ip +  ", " + this.getMinimum()  + ", " + this.getMedian()  + ", " + this.getAverage()+ ", " + this.getMaximum()  + ", " + this.getStdDev() + ", " + getNumSamples() + " " + this.samples + "\n";
	}
	...
	public Object getColumn(int columnIndex) {
		if (columnIndex==0) return this.countryCode + this.ip + "(" + this.testPointType + ")";
		if (columnIndex==1) return this.getMinimum();
		if (columnIndex==2) return this.getMedian();
		if (columnIndex==3) return this.getAverage();
		if (columnIndex==4) return this.getMaximum();
